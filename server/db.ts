...
import { 
  ...
  InsertAgentResult, agentResults, AgentResult,
...
export async function getUserReviews(userId: number, limit = 50) {
  const db = await getDb();
  if (!db) return [];
...
export async function getAgentResultsByReviewId(reviewId: number) {
  const db = await getDb();
  if (!db) return [];
...
export async function getFindingsByAgentResultId(agentResultId: number) {
  const db = await getDb();
  if (!db) return [];
